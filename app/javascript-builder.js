...
exports.build = function render(filesPatterns, opts) {
  ...
  var result = {},
    ...
    out = path.normalize(options.out || builderConf.defaultOutputFolder),
    excludes = options.excludes || builderConf.defaultExcludePattern;

  // find matching files
  return glob.getMatchingPaths(files)
    .then(function(foundFiles) {
      return filterOutExcludes(foundFiles, excludes, result);
    })
    .then(function(res){
      result = res;
      return Q.resolve(result.files);
    })
    .then(lint)
    .catch(function(report) {
      ...
      return uglify(result.files, mangle, out);
...
function filterOutExcludes(foundFiles, patterns, result) {
  var matchedFiles = foundFiles.map(function(file) {
  ...
  matchedFiles = matchedFiles.filter(Boolean);
